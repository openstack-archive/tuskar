...
.. note::
    ...
    requires that you use a version <1.70 or >= 1.7.2. This can be accomplished
    with `sudo pip install "tox<1.7.0,>=1.7.2"`.
...
config file in the ``database`` section and change
...
.. code-block:: bash
    ...
    $ curl -H 'Accept: application/json' http://0.0.0.0:8585/v2/roles/ | python -mjson.tool
