...
from tuskar.api.controllers.v3 import models
...
class PlansController(rest.RestController):
    ...
    test_plan = {
        'plan_file_1': {
            'contents': 'contents of plan file 1',
            'meta': {'file-type': 'type1'}
        },
        'plan_file_2': {
            'contents': 'contents of plan file 2',
            'meta': {'file-type': 'type2'}
        },
    }
    test_parameters = {
        'param1': 'value1',
    ...
    def get_all(self):
        ...
        #plans = tripleo_common.get_plan_list
        plans = [test_plan]
        transfer_plans = [models.Plan.from_dict(p)
                          for p in plans]
    ...
    def get_one(self, plan_name):
        ...
        if plan_name != 'overcloud':
            raise exception.PlanNotFound()
        plan = test_plan
        transfer_plan = models.Plan.from_dict(plan)
    ...
    def post(self, plan_name, transfer_plan):
        ...
        #try:
            # plan = tripleo_common.create_plan(plan_name, transfer_plan)
        #except SomeException
    ...
    def patch(self, plan_name, transfer_plan):
        ...
        #try:
        #    plan = tripleo_common.update_plan(plan_name, plan_files)
        #except SomeException
    ...
    @wsme_pecan.wsexpose([str],
                         str,
                         status_code=201)
    def get_environments(self, plan_name):
        """Retrieves plan environments
        ...
        #try:
        #    environments = tripleo_common.get_deployment_environments(plan_name)
        #except SomeException
        #    LOG.exception('Could not find plan environments: %s' % plan_name)
        #    raise exception.PlanNotFound()
        #return parameters
        if plan_name == 'overcloud':
            return []
    ...
    @wsme_pecan.wsexpose([str],
                         ...
                         body=[str],
                         status_code=201)
    def post_environments(self, plan_name, transfer_environments):
        """Patches existing plan parameters.

        :param plan_name: name of the plan
        :type  plan_name: str

        :param transfer_environments: data submitted by the user
        :type  transfer_environments: list

        :return: plan environments
        :rtype:  list
        """
        LOG.debug('Updating plan environments: %s' % transfer_environments)

        #try:
        #    params = tripleo_common.update_deployment_environments(plan_name, transfer_environments)
        #except SomeException
        #    LOG.exception('Could not update plan environments: %s' % plan_name)
        #    raise exception.PlanNotFound()
        #return params
        if plan_name == 'overcloud':
            return []
    ...
    def get_parameters(self, plan_name):
        ...
        #try:
        #    parameters = tripleo_common.get_deployment_parameters(plan_name)
        #except SomeException
    ...
    def post_parameters(self, plan_name, transfer_parameters):
        ...
        #try:
        #    params = tripleo_common.update_deployment_parameters(plan_name, transfer_parameters)
        #except SomeException
    ...
    def get_resource_types(self, plan_name):
        ...
        #try:
        #    resource_types = tripleo_common.get_deployment_plan_resource_types(plan_name)
        #except SomeException
    ...
    def post_resource_types(self, plan_name, transfer_resource_types):
        ...
        #try:
        #    params = tripleo_common.update_deployment_plan_resource_types(plan_name, transfer_resource_types)
        #except SomeException
    ...
    def roles(self, plan_name):
        ...
        #try:
        #    roles = tripleo_common.get_deployment_roles(plan_name)
        #except SomeException
