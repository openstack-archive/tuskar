...
class Base(wtypes.Base):
    ...
    @classmethod
    def from_db_model(cls, m, skip_fields=None):
        """Returns the database representation of the given transfer object."""
        skip_fields = skip_fields or []
        data = dict((k, v) for k, v in m.as_dict().items()
...
class OvercloudAttribute(Base):
    """Transfer object for overcloud attributes."""

    _db_class = db_models.OvercloudAttribute
...
class Overcloud(Base):
    ...
    def from_db_model(cls, db_overcloud, skip_fields=None):
        ...
        transfer_overcloud.attributes = [OvercloudAttribute.from_db_model(a)
                                         for a in db_overcloud.attributes]
        transfer_overcloud.counts = [OvercloudCategoryCount.from_db_model(c)
                                     for c in db_overcloud.counts]
        return transfer_overcloud

    def to_db_model(self, omit_unset=False, skip_fields=None):
        if self.attributes is wtypes.Unset:
            self.attributes = []
        ...
        db_model.attributes = [a.to_db_model() for a in self.attributes]
