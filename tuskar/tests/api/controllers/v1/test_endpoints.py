...
class TestEndpoints(api.FunctionalTest):
    ...
    def test_it_returns_the_overcloud_endpoints(self):
        ...
        tuskar.heat.client.HeatClient.get_stack().MultipleTimes().\
            AndReturn(heat_stack)
        self.mox.ReplayAll()
    ...
    def test_it_returns_404_for_nonexisting_overcloud(self):
        self.mox.StubOutWithMock(tuskar.heat.client.HeatClient, 'exists_stack')
        tuskar.heat.client.HeatClient.exists_stack().AndReturn(False)
        self.mox.ReplayAll()

        response = self.app.get('/v1/endpoints', expect_errors=True)
        self.assertEqual(response.status, '404 Not Found')

    def test_it_returns_no_endpoints_during_provisioning(self):
