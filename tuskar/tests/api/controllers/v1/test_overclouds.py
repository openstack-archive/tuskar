...
class TestOverclouds(api.FunctionalTest):
    ...
    def test_it_returns_the_overcloud_endpoints(self):
        ...
        tuskar.heat.client.HeatClient.get_stack(
            'stack_name').MultipleTimes().AndReturn(heat_stack)
        self.mox.ReplayAll()

        response = self.app.get('/v1/overclouds/stack_name/entrypoint')
    ...
    def test_it_returns_404_for_nonexisting_overcloud(self):
        ...
        tuskar.heat.client.HeatClient.exists_stack(
            'stack_name').AndReturn(False)
        self.mox.ReplayAll()
