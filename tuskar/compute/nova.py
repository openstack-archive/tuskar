...
class NovaClient(object):
    ...
    def register_resource_class_aggregate_flavor(self, resource_class):
        ...
        name = 'o_cloud_'+resource_class.service_type+'_'+resource_class.name
        #aggregate
        ...
        racks = db_api.Connection.get_racks_by_resource_class(db_api.get_backend(),
                                                              resource_class.id)
        for rack in racks:
    ...
    def unregister_resource_class_aggregate_flavors(self, resource_class):
        ...
        if resource_class.host_aggregate_id:
            ...
            for rack in racks:
                for node in rack.nodes:
                    self.remove_node_from_aggregate(resource_class.host_aggregate_id,
                                                    node.node_id)
    ...
    def create_flavor(self, flavor_data, flav_name, **kwargs):
        try:
            ...
            nova_flavor = self.nova_client.flavors.create(flav_name, ram, cpu, disk)
    ...
    def set_host_aggregate_metadata(self, host_aggregate_id, class_name):
        try:
            agg = self.nova_client.aggregates.set_metadata(host_aggregate_id, {'class' : class_name})
        except Exception as e:
            raise
        return agg
